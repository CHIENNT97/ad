...
install_simple_admin() {
    ...
    # CHANGED: wget -> curl
    curl -L -k -o /usrdata/simpleupdates/scripts/update_socat-at-bridge.sh $GITROOT/simpleupdates/scripts/update_socat-at-bridge.sh && chmod +x /usrdata/simpleupdates/scripts/update_socat-at-bridge.sh
    ...
    sleep 1
    # CHANGED: wget -> curl
    curl -L -k -o /usrdata/simpleupdates/scripts/update_simplefirewall.sh $GITROOT/simpleupdates/scripts/update_simplefirewall.sh && chmod +x /usrdata/simpleupdates/scripts/update_simplefirewall.sh
    ...
    set_simpleadmin_passwd
    # CHANGED: wget -> curl
    curl -L -k -o /usrdata/simpleupdates/scripts/update_simpleadmin.sh $GITROOT/simpleupdates/scripts/update_simpleadmin.sh && chmod +x /usrdata/simpleupdates/scripts/update_simpleadmin.sh
...
echo -e "\e[1;31m2) Installing tailscale from the $GITTREE branch\e[0m"
            ...
            mkdir /usrdata/simpleupdates/scripts > /dev/null 2>&1
            # CHANGED: wget -> curl
            curl -L -k -o /usrdata/simpleupdates/scripts/update_tailscale.sh $GITROOT/simpleupdates/scripts/update_tailscale.sh && chmod +x /usrdata/simpleupdates/scripts/update_tailscale.sh
...
configure_tailscale() {
    ...
    # CHANGED: wget -> curl
    curl -L -k -o tailscale-webui.service $GITROOT/tailscale/systemd/tailscale-webui.service
    curl -L -k -o tailscale-webui-trigger.service $GITROOT/tailscale/systemd/tailscale-webui-trigger.service
...
install_sshd() {
    ...
    mkdir /usrdata/simpleupdates/scripts > /dev/null 2>&1
    # CHANGED: wget -> curl
    curl -L -k -o /usrdata/simpleupdates/scripts/update_sshd.sh $GITROOT/simpleupdates/scripts/update_sshd.sh && chmod +x /usrdata/simpleupdates/scripts/update_sshd.sh
...
if echo "$ARCH" | grep -q "aarch64"; then
    # CHANGED: wget -> curl
    cd /tmp && curl -L -k -o RM55x_rcPCIe_toolkit.sh https://raw.githubusercontent.com/iamromulan/quectel-rgmii-toolkit/SDXPINN/RM55x_rcPCIe_toolkit.sh && chmod +x RM55x_rcPCIe_toolkit.sh && ./RM55x_rcPCIe_toolkit.sh && cd /
    exit 0
...
echo "                                                 :+##+.        "
    ...
    case $choice in
        ...
        11) 
            ...
            cd /usrdata/root/bin
            # CHANGED: wget -> curl with -O
            curl -L -k -O https://install.speedtest.net/app/cli/ookla-speedtest-1.2.0-linux-armhf.tgz
        ...
        12) 
            ...
            cd /usrdata/root/bin
            # CHANGED: wget -> curl
            curl -L -k -o fast https://github.com/ddo/fast/releases/download/v0.0.4/fast_linux_arm && chmod +x fast
            cd /
