...
if ! command -v curl >/dev/null 2>&1; then
    echo -e "\033[31mError: curl is required but not installed/found.\033[0m"
    exit 1
fi
...
echo -e '\033[32mInfo: Opkg package manager deployment (using curl)...\033[0m'
...
rm /bin/login 
# Check if /usr/bin/passwd exists before removing to avoid error
[ -f /usr/bin/passwd ] && rm /usr/bin/passwd
